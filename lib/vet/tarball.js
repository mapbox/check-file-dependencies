...
module.exports = function(module, dir, url, json) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    stream.on('entry', function(arg) {
      ...
      if (arg.path.indexOf('node_modules') !== -1) { arg.resume(); return; }
      if (parts[1] === 'package.json') {
        return children.push(vetPackageJSON(moduleDir, arg).catch((err) => {
          if (err.code === 'ENOENT' || err.message === 'Files dont match') return reject(new Error('Content of '+module+' does not match content of '+url));
          reject(err);
        }));
      };
      if (parts[parts.length-1] === '.gitignore') { arg.resume(); return; }
      if (parts[parts.length-1].slice(-3) !== '.js') { arg.resume(); return; }

      children.push(vetFile(moduleDir, arg).catch((err) => {
        if (err.code === 'ENOENT' || err.message === 'Files dont match') return reject(new Error('Content of '+module+' does not match content of '+url));
        reject(err);
      }));
    });
